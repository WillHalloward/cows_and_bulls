...
string generate_digits(){
  ...
  shuffle(answer.begin(), answer.end(),
          default_random_engine(static_cast<unsigned int>(chrono::system_clock::now().time_since_epoch().count())));
...
string player_guess() {
  ...
  while (true) {
    ...
    cin >> guess;
    if (guess.length() == NUM_LEN && guess.find_first_not_of(NUMBERS) == string::npos && check_dupe(guess)) {
      return guess;
    }
...
void cpu_cpu(){
  ...
  for (int i = 0; loops > i; i++) {
    ...
    while (true) {
      turn++;
      string guess = list[rand() % list.size()];
      printf("%s - Guess\n", guess.c_str());
      calculate_score(answer, guess, cows_bulls);
      if (cows_bulls.second == NUM_LEN) {
        break;
